...
(function () {
  ...
  const I18N = {
    zh: {
      ...
      "guide.title": "模型适配说明",
      "guide.close": "关闭",
      "guide.body": "<p>本插件只能使用能<strong>实时接收语音、并边听边返回字幕文字</strong>的<strong>语音（多模态）Realtime</strong> 模型。</p><p><strong>云端可用：</strong>通义 Qwen Realtime 语音（同传 / ASR / Qwen-Audio）、智谱 GLM-Realtime、OpenAI Realtime。</p><p><strong>本地 / 自部署可用：</strong></p><ul><li><strong>FunASR 流式识别</strong>（SenseVoice / Fun-ASR-Nano / paraformer-zh）：插件内置通道，默认 <code>ws://127.0.0.1:10095</code>；按 FunASR 官方 runtime 文档起 Docker 即可。</li><li><strong>huggingface/speech-to-speech</strong>（OpenAI Realtime 兼容网关）：<code>speech-to-speech serve --host 0.0.0.0 --stt parakeet-tdt --enable_live_transcription</code>，端点 <code>ws://&lt;主机IP&gt;:8765/v1/realtime</code>。</li><li>其它 ASR（faster-whisper / whisper.cpp / Parakeet-TDT / SenseVoice）需套一个 Realtime 网关（同上）。</li></ul><p><strong>不可用：</strong>纯文本 / 纯视觉模型、纯语音合成（TTS）、HTTP 上传式 ASR（非实时）——它们无法“听”实时音频，接上也不会有字幕。</p><p><strong>低延迟建议：</strong>中文直播只要中文字幕时，优先用 <strong>ASR 模型</strong>（云端 qwen3-asr-flash-realtime 或本地 FunASR）；同传翻译模型（livetranslate）按整句返回，延迟≈整句话时长。可再勾选“低延迟模式”按段推进。</p>",
      "support.btn": "支持本项目",
    ...
    en: {
      ...
      "guide.close": "Close",
      "guide.body": "<p>This plugin needs <strong>speech (multimodal) Realtime</strong> models that can <strong>receive streaming audio and return caption text on the fly</strong>.</p><p><strong>Cloud:</strong> Qwen Realtime speech (live translation / ASR / Qwen-Audio), Zhipu GLM-Realtime, OpenAI Realtime.</p><p><strong>Local / self-hosted:</strong></p><ul><li><strong>FunASR streaming ASR</strong> (SenseVoice / Fun-ASR-Nano / paraformer-zh): built-in channel, default <code>ws://127.0.0.1:10095</code>; start the Docker from the official FunASR runtime docs.</li><li><strong>huggingface/speech-to-speech</strong> (OpenAI-Realtime-compatible gateway): <code>speech-to-speech serve --host 0.0.0.0 --stt parakeet-tdt --enable_live_transcription</code>, endpoint <code>ws://&lt;host-ip&gt;:8765/v1/realtime</code>.</li><li>Other ASR (faster-whisper / whisper.cpp / Parakeet-TDT / SenseVoice) needs a Realtime gateway wrapper (as above).</li></ul><p><strong>Does not work:</strong> text/vision-only LLMs, pure TTS, or HTTP upload ASR (non-realtime) — they can't “hear” live audio, so no captions.</p><p><strong>Low latency tip:</strong> for Chinese livestreams that only need Chinese captions, prefer an <strong>ASR model</strong> (cloud qwen3-asr-flash-realtime or local FunASR); simultaneous-translation models (livetranslate) return a whole sentence at a time, so latency ≈ sentence length. You can also enable “low-latency mode” below to push results per segment.</p>",
